...
#define HEAD_TOP 0.5f        // 头管上端与上管之间的距离
#define STEM 0.2f            // 竖管
...
// 颜色
#define color_frame (50.0f / 255), (113.0f / 255), (174.0f / 255)  // 自行车整体框架 - 蓝色
...
// 初始化窗口内的场景
void init() {
    glClearColor(77.0f / 255, 183.0f / 255, 1.0f, 1.0f);  // 设置清屏颜色，蓝色背景
...
void landmarks(void) {
    ...
    for (int z = 0; z < 200; z++) {
        //float zStart = 30.0f - z * 10.0f;
        float zStart = -500.0f + z * 5.0f;
        for (int x = 0; x < 200; x++) {
            //float xStart = x * 10.0f - 30.0f;
            float xStart = -500.0f + x * 5.0f;
            if ((z % 2) ^ (x % 2)) {
                glColor4ub(27, 97, 26, 255);
            }
            else {
                glColor4ub(146, 242, 2, 255);
            }
            glVertex3f(xStart, -(RADIUS_WHEEL + bias), zStart);
            glVertex3f(xStart + 5.0f, -(RADIUS_WHEEL + bias), zStart);
            glVertex3f(xStart + 5.0f, -(RADIUS_WHEEL + bias), zStart - 5.0f);
            glVertex3f(xStart, -(RADIUS_WHEEL + bias), zStart - 5.0f);
        }
    }
    glEnd();
}
...
void change_size(GLint w, GLint h) {
     ...
     * gluPerspective (
        ...
        */
    gluPerspective(60.0f, (GLfloat)w / (GLfloat)h, 0.001, 50.0);
    glMatrixMode(GL_MODELVIEW);  // 对模型视图矩阵堆栈应用随后的矩阵操作
    glLoadIdentity();            // 重置当前指定的矩阵为单位矩阵
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Bicycle");  // 设置窗口标题
    init();                         // 初始化场景
